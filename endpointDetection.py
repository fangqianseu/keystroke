...
class EndPointDetect:
    ...
    def __init__(self, wave_data):
        ...
        self.energy = EndPointDetect.calEnergy(wave_data[:, 0])
        self.zeroCrossingRate = EndPointDetect.calZeroCrossingRate(wave_data[:, 0])
        self.wave_data_detected = EndPointDetect.endPointDetect(wave_data[:, 0], self.energy, self.zeroCrossingRate)
        self.endPoint_data = EndPointDetect.final_data(wave_data, self.wave_data_detected)
    ...
    def calZeroCrossingRate(wave_data):
        ...
        for i in range(len(wave_data)):
            ...
            if (i + 1) % setting.windows == 0:
                zeroCrossingRate.append(float(sum) / setting.windows)
                sum = 0
            elif i == len(wave_data) - 1:
                zeroCrossingRate.append(float(sum) / setting.windows)
    ...
    @staticmethod
    def final_data(wave_data, wave_data_detected):
        result = []

        m = 0
        N = wave_data_detected
        while m < len(N):
            begin = int(N[m] * setting.windows)
            end = int(N[m + 1] * setting.windows)

            m = m + 2
            # 滤去长度过短的片段
            if end - begin < 0.05 * 48000:
                continue
            result.append(wave_data[begin:end, :])
